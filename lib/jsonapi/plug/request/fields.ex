defmodule JSONAPI.Plug.Request.Fields do
  ...
  def call(
    ...
    Conn.put_private(
      conn,
      :jsonapi,
      %JSONAPI{jsonapi | fields: parse_fields(jsonapi, query_params["fields"])}
    )
  end

  defp parse_fields(%JSONAPI{fields: fields}, nil), do: fields

  defp parse_fields(%JSONAPI{view: view}, fields) when is_map(fields) do
    Enum.reduce(fields, %{}, fn {type, value}, fields ->
      ...
      requested_fields =
        try do
          ...
          |> Enum.into(MapSet.new(), &String.to_existing_atom(JSONAPI.recase(&1, :underscore)))
        rescue
          ArgumentError ->
  ...
  defp parse_fields(%JSONAPI{view: view}, fields) do
